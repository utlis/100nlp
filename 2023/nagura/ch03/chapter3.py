...
for m in re.finditer(r'(=+)([^ =]+)(=+)', uk_text): # ^は先頭、$は末尾、また、^を使うと否定（拾わないでほしい文字）になる
    sec_name = m.group(2)
...
# pattern = re.compile(r'^\|([^=\|].+?)=(.+?\S)$', re.MULTILINE + re.S) #\Sはスペースなしの任意の文字 ,なんで?が必要かが分からない
pattern = re.compile(r'^\|(.*)=(.*)$', re.MULTILINE + re.S) #\Sはスペースなしの任意の文字 ,なんで?が必要かが分からない
